...
| `db.ts` | All SQLite: spaces, conversations, messages, tasks, roles, config, preferences |
...
- `space_preferences` — Per-space assistant preferences (chat-managed)
...
| `/api/prefs` | GET/PUT | List/set preferences |
| `/api/prefs/:key` | GET/DELETE | Get/delete one preference |
...
| `/api/tts/synthesize` | POST | Cloud TTS MP3 (`tts.synthesize` perm; host Google/Azure credentials) |
...
- **Built-in**: `tasks`, `roles`, `permissions`, `config`, `prefs`, `spaces`, `conversations`, `stop`, `compact` — HTTP calls to host API
